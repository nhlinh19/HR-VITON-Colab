...
def get_opt():
    ...
    parser.add_argument('--tocg_checkpoint', type=str, default='./model/mtviton.pth', help='tocg checkpoint')
    parser.add_argument('--D_checkpoint', type=str, default='./model/discriminator_mtviton.pth', help='D checkpoint')
...
def test(opt, test_loader, board, tocg, D=None):
    ...
    for inputs in test_loader.data_loader:
        ...
        with torch.no_grad():
            # inputs
            # input1 = [cloth image, cloth mask] = [c, cm]
            input1 = torch.cat([c_paired, cm_paired], 1)
            # input2 = [parese agnostic, densepose]= [Sa, P]
        ...
        for i in range(c_paired.shape[0]):
            grid = make_grid(
                [   
                    #input of condition (c, cm, parse_agnostic, denspose)
                    (c_paired[i].cpu() / 2 + 0.5), 
                    (cm_paired[i].cpu()).expand(3, -1, -1), 
                    visualize_segmap(parse_agnostic.cpu(), batch=i), 
                    ((densepose.cpu()[i]+1)/2),

                    # Groundtruth cloth segmentation 
                    (im_c[i].cpu() / 2 + 0.5), 
                    parse_cloth_mask[i].cpu().expand(3, -1, -1), 
                    # Wrap cloth (Output) 
                    (warped_cloth_paired[i].cpu().detach() / 2 + 0.5), 
                    (warped_cm_onehot[i].cpu().detach()).expand(3, -1, -1), #Wrap cloth mask

                    visualize_segmap(label.cpu(), batch=i), # Ground truth segmentation map (Sa)
                    visualize_segmap(fake_segmap.cpu(), batch=i), # Output segmentation map
                    (im[i]/2 +0.5), # Input image
                    (misalign[i].cpu().detach()).expand(3, -1, -1) # Difference between fake_segment and fake_clothMask
                ],
                nrow=4
            )
