...
class CPDatasetTest(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        # base setting
        self.opt = opt
        self.root = opt.dataroot # data folder
        self.datamode = opt.datamode # train or test or self-defined
        self.data_list = opt.data_list # test_pairs.txt
        self.fine_height = opt.fine_height # 256
        self.fine_width = opt.fine_width # 192
        self.semantic_nc = opt.semantic_nc # 13
        self.data_path = osp.join(opt.dataroot, opt.datamode) # data/test
        self.transform = transforms.Compose([  \
                transforms.ToTensor(),   \
                transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
        ...
        with open(osp.join(opt.dataroot, opt.data_list), 'r') as f:
            for line in f.readlines():
                im_name, c_name = line.strip().split() # person name | cloth name
                im_names.append(im_name)
                c_names.append(c_name)

        self.im_names = im_names #person name
        self.c_names = dict()
        self.c_names['paired'] = im_names # cloth name of true person ???? 
        self.c_names['unpaired'] = c_names # cloth name
    ...
    def __getitem__(self, index):
        ...
        c = {}
        cm = {}
        for key in self.c_names: # key = paired | unpaired
            c_name[key] = self.c_names[key][index]
            # c["paired"], c["unpaired"] cloth image
            c[key] = Image.open(osp.join(self.data_path, 'cloth', c_name[key])).convert('RGB') # 
            c[key] = transforms.Resize(self.fine_width, interpolation=2)(c[key])
            # cm["paired"] cm["unpaired"] cloth mask
            cm[key] = Image.open(osp.join(self.data_path, 'cloth-mask', c_name[key]))
            cm[key] = transforms.Resize(self.fine_width, interpolation=0)(cm[key])
        ...
        im_parse = Image.open(osp.join(self.data_path, 'image-parse-v3', parse_name)) # image parse: segmentation of the body parts
        ...
        parse_map = torch.FloatTensor(20, self.fine_height, self.fine_width).zero_()
        parse_map = parse_map.scatter_(0, parse, 1.0)
        new_parse_map = torch.FloatTensor(self.semantic_nc, self.fine_height, self.fine_width).zero_() # semantic_nc: number of labels
        ...
        # load image-parse-agnostic
        image_parse_agnostic = Image.open(osp.join(self.data_path, 'image-parse-agnostic-v3.2', parse_name)) # Segmentation without cloth
        image_parse_agnostic = transforms.Resize(self.fine_width, interpolation=0)(image_parse_agnostic)
        parse_agnostic = torch.from_numpy(np.array(image_parse_agnostic)[None]).long()
        image_parse_agnostic = self.transform(image_parse_agnostic.convert('RGB'))
        ...
        # load pose points
        pose_name = im_name.replace('.jpg', '_rendered.png')                        # skeleton pose
        ...
        # load densepose
        densepose_name = im_name.replace('image', 'image-densepose')                # body pose
        ...
        result = {
            ...
            'im_name':  im_name,    # for visualization or ground truth
            # intput 1 (clothfloww)
            'cloth':    c,          # for input cloth image
            'cloth_mask':     cm,   # for input cloth mask
            # intput 2 (segnet)
            'parse_agnostic': new_parse_agnostic_map,
            'densepose': densepose_map, # image densepose
            'pose': pose_map,       # for conditioning image pose
            # GT
            'parse_onehot' : parse_onehot,  # Cross Entropy
            'parse': new_parse_map, # GAN Loss real
            'pcm': pcm,             # L1 Loss & vis
            'parse_cloth': im_c,    # VGG Loss & vis
            # visualization
            'image':    im,         # for visualization person image
            }
